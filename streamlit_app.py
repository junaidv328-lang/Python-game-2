...
with data_tab2:
    ...
    with st.form("angel_login_form", clear_on_submit=False):
        api_key = st.text_input("API Key",
            value=st.secrets.get('angel_one', {}).get('api_key', '') if secrets_available else '',
            type='password')
        client_id = st.text_input("Client ID",
            value=st.secrets.get('angel_one', {}).get('client_id', '') if secrets_available else '')
        password = st.text_input("Password",
            value=st.secrets.get('angel_one', {}).get('password', '') if secrets_available else '',
            type='password')
        totp_key = st.text_input("TOTP Secret Key",
            value=st.secrets.get('angel_one', {}).get('totp_key', '') if secrets_available else '',
            type='password',
